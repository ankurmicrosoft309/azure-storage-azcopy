...
| Source endpoint kind | `SourceEndpointKind` | Independent source hostname classification. Explicit `.privatelink.` hosts are private-endpoint; no DNS or routing inspection occurs. Missing historical values remain unknown. |
...
| Data by source endpoint kind | Logical payload volume by independent source hostname classification; historical missing values remain unknown. |
