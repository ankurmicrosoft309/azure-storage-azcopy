...
| --- | --- | --- |
| AzCopy `azcopy.job.finished` events | Event time in the selected range | Destination account name, topology, counters, sampling rate, and sender geography. |
...
- `Destination enrichment mapping coverage` reports weighted jobs and bytes for mapped and missing states instead of silently dropping unmatched telemetry.
...
| --- | --- |
| How to read this dashboard | Visible interpretation, sampling, and enrichment guidance. |
| Selected-range data totals | Observed unique jobs plus inverse-sampling estimates for jobs, logical payload bytes, and completed payload objects. |
| Current versus previous month | Calendar-month comparison while honoring topology and enrichment filters. |
| Monthly estimated data transferred | Monthly logical successful payload volume. |
| Monthly estimated completed objects | Monthly completed payload objects excluding folder-property operations. |
| Estimated data by source-target pair | Volume grouped by AzCopy source and destination service type. |
| Estimated completed objects by source-target pair | Completed objects grouped by source and destination service type. |
| Estimated data by destination service type | Blob, BlobFS, File, FileNFS, Local, and other AzCopy destination types. |
| Estimated data by source mount type | Local disk, NAS SMB/NFS, cloud, and other source classifications. |
| Estimated data by destination endpoint kind | Public-hostname versus explicit private-link endpoint classification. |
| Estimated data by telemetry-sender country/region | Client IP geography, explicitly separate from Storage resource region. |
| Recent observed finished jobs | Latest 200 unique jobs with destination enrichment values and mapping statuses. |
| Estimated data by destination Storage account kind | Current ARM kind such as `StorageV2`; unmatched accounts remain visible. |
| Estimated data by destination Storage SKU | Current ARM SKU such as `Standard_RAGRS`; unmatched accounts remain visible. |
| Estimated data by destination redundancy | Historical XStore redundancy for the activity day. |
| Estimated data by destination namespace | Historical HNS/FNS state for the activity day. |
| Estimated data by destination access tier | Historical XStore access tier for the activity day. |
| Estimated data by destination account class | Historical XStore billing/account class, deliberately distinct from ARM kind and SKU. |
...
## Counting, sampling, and resume semantics
...
- Estimated totals use `Weight = 1 / SamplingRate` when `0 < SamplingRate <= 1`.
- Observed job counts and the recent-jobs table are received telemetry records, not population estimates.
