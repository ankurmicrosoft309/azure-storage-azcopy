...
The import contains 45 tiles across six pages, including six visible field guides. It uses an XStore dashboard data source to execute explicit cross-cluster queries against Application Insights. If the source needs reconnecting, use:
...
Each page ends with an always-displayed **Field definitions** Markdown panel for
the relevant metrics and dimensions. Native tile descriptions remain available.
The guides cover destination classification, scanned versus scheduled bytes,
source-relative payload depth, hardlink conversion, HTTP attempts versus IOPS,
and the environment-variable heuristic behind invocation context. The same
shared definitions are included in relevant ADX and Grafana dashboard variants.
Existing data panels are not moved. These guides become visible in deployed
dashboards only after the updated artifacts are imported or published.
...
After regenerating all dashboard variants, check descriptions and guide placement
offline with `./dashboards/test-metric-descriptions.ps1`. During review, add
`-BaselineRef <commit>` to verify that original queries, IDs, layouts, and data
sources are unchanged; only descriptions, added guides, and guide counts may differ.
