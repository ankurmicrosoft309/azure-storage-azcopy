...
This manual Windows/Linux test runs the same built AzCopy executable against real Azure
...
Add `-PerformanceWorkload long -PerformancePairs 3` for sustained transfers:
one 2 GiB blob and 4,096 blobs of 256 KiB each (1 GiB total). Both modes use a
128 Mbps cap, so this preset measures sustained-run overhead, not maximum
throughput. Three pairs plus warm-ups download 24 GiB total. Fixture upload and
SHA-256 validation are outside the timed CLI run. Fixtures are streamed through a
temporary file instead of retaining a dataset-sized parent-process buffer.
The long preset allows six minutes per process, `15 + 12 * pairs` minutes for the
experiment, and five additional minutes for the test runner.

Use `-PerformanceWorkload large-only` to run only the 2 GiB single-blob case at
128 Mbps. `-PerformanceBufferGB 0.125` optionally sets the same 128 MiB in-flight
buffer budget in both modes for a diagnostic control; zero uses the product
default. This is not a cap on total process memory or cached idle slices.
The [large-file memory investigation](results/cli-large-investigation-2026-09-09/README.md)
contains Windows profiles, the buffer-control experiment, and native Linux WSL 2
results. The shared transfer test has moved to
`azcopy/telemetry_cli_perf_transfer_test.go`.

The [Azure Windows VM follow-up](results/cli-azure-vm-2026-09-09/README.md) ran
the same binary on a disposable Windows Server 2022 Core VM in East US 2 EUAP,
using managed identity and a fixed 128 MiB buffer budget in both modes. Both
unprofiled and profiled phases passed. The large local image-residency increase
did not reproduce: the unprofiled paired average-working-set increase was
3.66 MiB, and profiled executable residency stayed around 16 MiB in both modes.
The report records environment differences, raw evidence, and verified deletion
of all temporary cloud resources and external role assignments.

The [local 64 MiB stage-removal investigation](results/cli-local64-ablation-2026-09-09/README.md)
subsequently isolated the local increase to the real IMDS detection request.
Skipping only IMDS removed the increase while telemetry delivery remained active;
performing the request and forcing its result false still reproduced it. Delay,
transport-only, connection cleanup, and unprofiled controls are recorded in that
report. The Windows paging actor remains unidentified, and production code was
not changed during that investigation. `-PerformanceWorkload 64m-only` runs the normal uncapped single-file
workload without diagnostic stage removals.

The [firmware detection regression](results/cli-firmware-2026-09-10/README.md)
then verified the production replacement: local SMBIOS/sysfs detection with no
telemetry IMDS fallback. In an unprofiled four-condition comparison, enabled
median average working set fell from 98.35 to 56.13 MiB and peak working set from
197.54 to 119.58 MiB, with peak private commit still near 141 MiB. All 16 transfers
passed, all eight enabled processes sent both lifecycle events, and the native
Windows probe still detected this Azure public-cloud host. This is a local
three-round regression result, not a universal overhead guarantee.

The one-off ablation runner and disposable-VM investigation tooling are not part
of the maintained test suite. Their sources are preserved in the local recovery
ref `backup/telemetry-before-rework-20260910`; recorded results remain historical
evidence. The normal performance runner and native Linux launcher are retained.

### Linux in WSL

The PowerShell runner remains Windows-only. The opt-in
`testSuite/telemetry-wsl-perf.sh` launcher runs prebuilt native Linux binaries
`bin/azcopy` and `bin/azcopy.test` under `AZCOPY_WSL_PERF_ROOT`. Build these with
the same Go toolchain (`GOOS=linux GOARCH=amd64 CGO_ENABLED=0`, test build tags
`telemetrylive,telemetryperf`) and put the root and output directories on Linux
ext4, not `/mnt/c` or the WSL `/tmp` tmpfs. The launcher uses the installed Windows
Azure CLI through `telemetry-wsl-az.sh` and the existing login; no tokens are copied
into files. Set `WSL_AZURE_CONFIG_DIR` to the WSL path of the Windows CLI config.
This bridge expects the standard Windows Azure CLI installation path used in the
script; other installations need that path adjusted.

Set the subscription, resource group, suffix, storage account, and output through
the existing `AZCOPY_LIVE_TELEMETRY_*` environment variables. Explicitly set
`AZCOPY_WSL_GRANT_STORAGE_PERMISSION=1` for the temporary container-scoped grant,
or `0` to use existing permissions. The workload is large-only, with three pairs
and profiling off by default. `AZCOPY_CLI_TELEMETRY_PERF_BUFFER_GB` optionally
applies the same buffer control. Authentication still uses a Windows subprocess;
CPU and memory measurements cover only the native Linux AzCopy process.

Linux memory uses `/proc/<pid>/status`: `VmRSS`, `VmHWM`, and `RssAnon`. Reported
average RSS/anonymous RSS are time-weighted over valid samples. Anonymous RSS is
not Windows private commit. Profiling optionally samples `smaps_rollup` for PSS,
private RSS, and swap. Linux file-cache consumption outside process RSS is not
included; WSL is not a bare-metal Linux performance equivalence claim.
...
- Average working set and private committed memory are time-weighted using the
  trapezoidal integral of current counters over valid live samples, divided by
  `MemoryObservedSeconds`. Zero post-exit counters are excluded. The nominal
  sampling period is 10 ms; elapsed sample times determine the weights. This
  covers startup, transfer, and exit flush, not only steady-state transfer.
  Per-mode summaries are medians of the individual process averages. No first or
  last unobserved interval is extrapolated; compare observed duration with wall
  time in the retained trials to assess coverage.
...
collection and local Azure public-cloud detection through the chassis asset tag.
Windows reads raw SMBIOS with GetSystemFirmwareTable; Linux reads the bounded
chassis-tag sysfs file. Missing/unsupported detection returns false, with no IMDS
fallback. Historical results above used the former IMDS probe. Windows
