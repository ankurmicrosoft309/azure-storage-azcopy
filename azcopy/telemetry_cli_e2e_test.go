...
func newCLIE2EBlob(t *testing.T) (string, []byte) {
	t.Helper()
...
func TestTelemetryCLIEndToEnd(t *testing.T) {
	if os.Getenv("AZCOPY_RUN_TELEMETRY_CLI_E2E") != "1" {
		t.Skip("opt-in local CLI E2E; set AZCOPY_RUN_TELEMETRY_CLI_E2E=1")
	}
	extension := ""
	...
	t.Run("cancellation and resume", func(t *testing.T) {
		testCLIE2ECancelResume(t, executable)
	})
	t.Run("benchmark download", func(t *testing.T) {
		testCLIE2EBenchmarkDownload(t, executable)
	})
	t.Run("benchmark upload cleanup", func(t *testing.T) {
		testCLIE2EBenchmarkCleanup(t, executable)
	})
